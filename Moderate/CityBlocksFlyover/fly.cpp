...
void vectorize_intlist( vector<unsigned> & v, string & s) {
    istringstream iss(s);
    unsigned num;
    while (iss >> num) {
	v.push_back(num);
...
int main(int argc, char *argv[]) {
	...
	unsigned blankpos = (unsigned) line.find(' ');
	// for loop walking line once preferred if line can be long
	replace(line.begin(), line.end(), '(', ' ');
	replace(line.begin(), line.end(), ')', ' ');
        replace(line.begin(), line.end(), ',', ' ');

	string xstr = line.substr(0, blankpos);
	string ystr = line.substr(blankpos + 1);

	vector<unsigned> xvec, yvec;
	vectorize_intlist( xvec, xstr);
	vectorize_intlist( yvec, ystr);

	unsigned last_x = (unsigned) xvec.size() - 1;
	unsigned last_y = (unsigned) yvec.size() - 1;
	float slope = 
	    (float(yvec[last_y]) - yvec[0]) / (xvec[last_x] - xvec[0]);
	unsigned blocks_overflown = 0;
	...
	for (unsigned y = 0; y <= last_y - 1; ++y) {
	    for (unsigned x = 0; x <= last_x - 1; ++x) {
		if ( slope * xvec[x] < yvec[y + 1] &&
		     slope * xvec[x + 1] > yvec[y] )
		    ++blocks_overflown;
	    }
	}
	cout << blocks_overflown << endl;
    } // while getline
}
